...
#define LIST_TAKE_NTH(X,N) _LIST_TAKE_NTH_EVAL(_LIST_TAKE_NTH_PREAMBLE(U32_EQ(N,U32_0),X,N))
...
#define _LIST_TAKE_NTH_PREAMBLE(G,X,N) DEFER(CAT(_LIST_TAKE_NTH_P, G))(X,N)
#define _LIST_TAKE_NTH_P0(X,N)  _LIST_TAKE_NTH(LIST_IS_NIL(X),X,N,N)
#define _LIST_TAKE_NTH_P1(X,N)  LIST_NIL

#define _LIST_TAKE_NTH(NX,X,N0,N) DEFER(CAT(_LIST_TAKE_NTH_X, NX))(X,N0,N)
#define _LIST_TAKE_NTH_X1(X,N0,N) LIST_NIL
#define _LIST_TAKE_NTH_X0(X,N0,N) DEFER(CAT(_LIST_TAKE_NTH_Y,U32_EQ(N,U32_1)))(X,N0,N)
#define _LIST_TAKE_NTH_Y1(X,N0,N) (LIST_FIRST(X),_LIST_TAKE_NTH_Z(LIST_REST(X),N0,N0))
#define _LIST_TAKE_NTH_Y0(X,N0,N) _LIST_TAKE_NTH_Z(LIST_REST(X),N0,U32_SUB(N,U32_1))

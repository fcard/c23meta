...
#define LIST_SPLIT(P,X) _LIST_SPLIT_C(P,X)
#define _LIST_SPLIT_C(P,X)  _LIST_SPLIT_EVAL(_LIST_SPLIT(P,LIST_NIL,LIST_IS_NIL(X),X))
...
#define _LIST_SPLIT(P,R,N,X) CAT(_LIST_SPLIT_X, N)(P,R,X)
#define _LIST_SPLIT_X1(P,R,X) LIST(LIST_REVERSE(R),LIST_NIL)
#define _LIST_SPLIT_X0(P,R,X) CAT(_LIST_SPLIT_Y,P(LIST_FIRST(X)))(P,R,X)
